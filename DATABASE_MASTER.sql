...
-- ==========================================
-- This script sets up EVERYTHING: Profiles, Posts, Likes, Comments, Comment Likes, and Follows.
...
SELECT public.drop_all_policies('comment_likes');
...
-- ⚡ NEW: Comment Likes table (was missing!)
...
-- Comments: authenticated can insert, owners can delete
CREATE POLICY "Authenticated users can comment" ON public.comments FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Users can delete own comments" ON public.comments FOR DELETE USING (username = (SELECT username FROM profiles WHERE id = auth.uid()));

-- Comment Likes: owners can toggle
